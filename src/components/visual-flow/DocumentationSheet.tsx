        ...
        content: () => (
            <div className="space-y-4 text-sm">
                ...
                <ul className="list-disc pl-5 space-y-2">
                    ...
                    <li><strong>Join From Different Entries:</strong> A `Join` block cannot merge two token streams that originate from two different `Entry` blocks, even if the two branches were connected correctly one at a time.</li>
                    <li><strong>Dangling Edges:</strong> Detects connections whose source or target no longer points to an existing block, which can happen if a model was hand-edited or imported from another tool.</li>
                    <li><strong>Dangling References:</strong> Detects a selected `Token`, `Pool`, or `Variable` (in an `Entry`, `Mint`, `Pool`, `Deposit`, `Withdraw`, or `Set` block) that no longer matches any existing token, pool, or variable, typically because it was renamed or deleted after being selected.</li>
                    <li><strong>Duplicate Names:</strong> Flags the same name being reused across multiple tokens, multiple pools, or multiple variables, since this makes any reference by name ambiguous.</li>
                    <li><strong>Handle Type Mismatch:</strong> Checks that every connection links compatible handle types (e.g. you cannot connect a `boolean` output to a `number` input), re-verified on every edge already present in the model, not just when you draw it.</li>
                    <li><strong>Single Edge Per Output:</strong> Each output handle, including `Type Block` outputs like `Number` or `Address`, can feed at most one connection. If the same value or stream is needed in more than one place, duplicate the block instead of connecting it to multiple targets.</li>
                    <li><strong>Single Edge Per Input:</strong> Symmetrically, each input handle can receive at most one incoming connection, since two simultaneous values feeding a single input would be ambiguous.</li>
                    <li><strong>Join Token Mismatch:</strong> Detects a `Join` block combining two streams that carry two different tokens by identity (not just two different `Entry` blocks), which would make the resulting stream's token undefined.</li>
                    <li><strong>Join / Split on Non-Fungible Tokens:</strong> `Join` sums quantities and `Split` divides a quantity by a ratio, so neither operation is meaningful on a Non-Fungible token, which is identified by a unique id rather than an amount.</li>
                </ul>
